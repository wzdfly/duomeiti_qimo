(function initFishermanGameLogic(){
...
function createFishes(n){
  ...
  for(let i=0;i<n;i++){
    ...
    arr.push({x,y,dir,speed:s,radius:r,caught:false});
  }
  return arr;
}
...
function endGame(){
  ...
  canvas.onclick=function(e){
    ...
    else if(back.isClicked(x,y)){ location.href='../index.html'; }
  };
  canvas.onmousemove=function(e){
    const {x,y}=windowToCanvas(canvas,e.clientX,e.clientY);
...
window.startFishingGame=function(){ startGame(); };
})();
