...
// Config for Medium Mode
const SWING_SPEED_FACTOR = 3; // Faster swing
const DROP_SPEED = 400;
const RETRIEVE_SPEED = 500;
  ...
  fishes=createFishes(8); // More fish
  paused=false;
  ...
  const bw=120,bh=50; 
  const startX = W/2 - (bw*3 + 20)/2 + 100; 
...
function createFishes(n){
  ...
  for(let i=0;i<n;i++){
    ...
    // Medium: Faster speed (80-120), Medium radius (35-50)
...
function update(dt){
  ...
  if(timeLeft<=0){
     ...
     try {
       ...
       scores.medium = scores.medium.slice(0, 5); 
  ...
  switch(hook.state) {
      ...
      case 'retrieve':
          ...
          if(hook.length <= 30) {
              hook.length = 30;
              if(hook.caughtFish) {
                  score += 20; // More points for medium
                  let f = hook.caughtFish;
                  f.caught = false;
                  f.y=H*0.35+Math.random()*H*0.55;
                  f.dir=Math.random()<0.5?-1:1;
                  f.x=f.dir<0?W+60:-60;
                  f.speed=80+Math.random()*40; 
...
function drawHUD(){
  ...
  ctx.textAlign='left';
  ctx.fillText('中级模式 - 得分: '+score,20,50);
  ctx.textAlign='right';
