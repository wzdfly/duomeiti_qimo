(function initFishermanMediumGame(){
...
let waveT=0;
  ...
  fishes=createFishes(9); // Reduced from 12
  paused=false;
...
function createFishes(n){
  ...
  for(let i=0;i<n;i++){
    ...
    // Faster speed (100-180), Medium radius (20-30)
...
function update(dt){
  ...
  if(timeLeft<=0){
     ...
     try {
       ...
       scores.medium.push({score: score, date: new Date().toLocaleString()});
       scores.medium.sort((a,b)=>b.score-a.score);
       scores.medium = scores.medium.slice(0, 5); // Keep top 5
  ...
  if(hook.drop){
    ...
    for(const f of fishes){
      ...
      if(dx*dx+dy*dy<(f.radius+8)*(f.radius+8)){
        ...
        f.x=f.dir<0?W+60:-60;
        f.speed=100+Math.random()*80;
        f.radius=20+Math.random()*10;

        f.caught=false; // Just to be safe, though we don't set it to true anymore
        score+=20; // More points for faster fish
        hook.drop=false;
        hook.length=0;
        break;
      }
...
function drawScene(){
  ...
  for(const f of fishes){
    ...
    ctx.fillStyle='#ff7043'; // Default fish color
    ctx.beginPath();
...
function drawHUD(){
  ...
  ctx.textAlign='left';
  ctx.fillText('中级模式 - 得分: '+score,20,40);
  ctx.textAlign='right';
...
function drawEnvironment(ctx, W, H, t) {
    ...
    for(let k=0; k<12; k++){
        let swX = (W/13) * (k+1) + Math.sin(k)*30; 
        let swH = 90 + Math.sin(k*132)*25; 
...
window.startMediumGame=function(){ startGame(); };
})();
